...
**Dashboard Banner** (shown until photo uploaded):
```
🔒 Upload a photo to start browsing profiles
[Upload Photo Button]
```

**Search Page** (if accessed before photo upload):
```
📷 Complete your profile to view matches
You need at least 1 profile photo to view other members.
[Upload Photo Button]
```

**Profile Card** (photo section, if viewer has no photo):
```
🔒 Photos locked
Upload your photo to view theirs
[Upload Photo Button]
```
...
- **Chat**: Supabase Realtime-based messaging with file sharing (Phase 3/4)
...
CREATE TABLE messages (
  ...
  file_url VARCHAR(500), -- Signed URL (generated by API)
...
- **Message retention**: Keep 6–12 months of chat history in the primary `messages` table. Run the weekly cleanup job below once growth accelerates; export to Cloudinary only if long-term archives are required.
...
**Week 5: Reciprocity (MVP Photo Gate)**
- Day 22-24: Implement simple photo gate (viewer must have ≥1 approved photo to view others’ photos); respect photo privacy
- Day 25-26: UI prompt “Upload a photo to view” + testing
- Day 27-28: Telemetry + admin visibility (basic only)
- **Checkpoint:** Photo-gated reciprocity live; full bundles deferred to Phase 2

### **Phase 3 – Engagement (Weeks 6-7)** [SIMPLIFIED FOR AI]
**Week 6: Notifications & Engagement (no chat)**
...
- **Checkpoint:** Engagement emails working end-to-end (no realtime chat)

**Week 7: Polish & Beta Ops (no Admin panel)**
- Day 36-38: UX polish, loading states, empty states
- Day 39-40: Error handling + Sentry breadcrumbs
- Day 41-42: Basic ops widgets (read-only metrics) optional
- **Checkpoint:** MVP ready without Admin panel; Admin deferred to Phase 2
...
- **Use**: Review before Day 1, and reference during Week 7 deployment.
...
### 26.5 Reciprocity Specification
- **File**: `RECIPROCITY-SPEC.md`
- **Purpose**: Detailed specification of the reciprocity engine (simplified to photo gate for MVP).
