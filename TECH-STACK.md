...
| Image Handling | **Cloudflare R2 (private, signed URLs)** | Photo storage via S3-compatible API; accessed through signed URLs generated by Vercel API (setup in Week 3) |
...
| Storage | **Cloudflare R2 (private bucket)** | Photo/document storage with signed URL access via Vercel API (setup in Week 3) |
...
## 6. Media Handling
- **Upload**: Client-side compression → upload via Vercel API route → store in Cloudflare R2 (private) with signed URL access; PNG/GIF/JPG/JPEG/WebP up to 15 MB (compressed client-side)
- **Plan Quotas**: Free (1 profile + 1 album slot); Paid (1 profile + 9 album + 3 family/group slots)
- **Processing (SIMPLIFIED for MVP)**: 
  - **Phase 1 (Week 3)**: Client-side compression using `browser-image-compression` (max 1200×1500, <500KB), upload to R2 via Vercel API
  - **Phase 2 (Post-launch)**: Add server-side Sharp pipeline for watermarking if needed (still via Vercel API → R2)
...
```typescript
STORAGE_CONFIG = {
  mvp: 'Cloudflare R2 (private bucket, signed URLs via Vercel API; setup in Week 3)',
  ...
  migration_target: 'Remain on R2; consider Cloudflare Images for optimization at scale',
...
CREATE TABLE photos (
  ...
  storage_path VARCHAR(255) NOT NULL, -- R2 object key (provider-agnostic)
  is_approved BOOLEAN DEFAULT TRUE, -- Auto-approve as requested
...
- **Message retention**: Keep 6–12 months of chat history in the primary `messages` table. Run the weekly cleanup job below once growth accelerates; export to R2 only if long-term archives are required.
- **Media handling**: Store photos and attachments in Cloudflare R2; persist only metadata and `storage_key` in Postgres. Access via signed URLs generated by Vercel API.
...
- **Storage Monitoring**: Cloudflare R2 usage via API/dashboard (bucket size, object count)
...
export default async function handler(req, res) {
  try {
    ...
    // Storage check (Cloudflare R2 - fetch bucket stats via R2 API/SDK)
    // Example: list objects and sum sizes or use head-bucket stats if available
    const { files, sizeMb } = await getR2BucketStats(); // implement using @aws-sdk/client-s3 against R2
...
3. **Alert Thresholds**
   - Storage usage (Cloudflare R2) – set an early MVP alert around 1 GB (budget-based)
...
| Storage usage | Cloudflare R2 usage | MVP: alert ~1 GB (budget-based) | Clear stale assets or adjust plan/alerts |
```typescript
...
- **Server-side image processing (Sharp)**: Build `/api/upload/photo` route handling resize → WebP/JPEG conversion → watermark → Cloudflare R2 upload; adjust client flow to call API and add telemetry.
...
- Day 8-9: Photo upload (client-side compression with `browser-image-compression`, Vercel API → Cloudflare R2 private bucket with signed URL response; defer server watermarking)
...
R2_BUCKET_NAME=ccm-photos
R2_PUBLIC_DOMAIN=https://<custom-domain-or-r2-endpoint>
...
- Week 3: Photo upload wired (Vercel API → R2), introduce Zustand where cross-feature state emerges.
